-- 交換完了時にtotal_tradesをインクリメントするよう更新
-- また、total_stickersも再計算する
...
BEGIN
  ...
  -- 両ユーザーのtotal_tradesをインクリメント
  UPDATE profiles
  SET total_trades = total_trades + 1
  WHERE id IN (v_session.requester_id, v_session.responder_id);

  -- 両ユーザーのtotal_stickersを再計算（ユニークなsticker_idの数）
  UPDATE profiles p
  SET total_stickers = (
  ...
  WHERE p.id IN (v_session.requester_id, v_session.responder_id);
