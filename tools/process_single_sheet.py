...
def process_marshmallow_only(char_name: str, marshmallow_path: str, start_num: int = 16):
    """マシュマロシールのみ処理（既存ボンドロは維持）"""

    print(f"\n{'='*60}")
    print(f"[MARSHMALLOW ONLY] {char_name}")
    print(f"{'='*60}")

    marsh_dir = os.path.join(OUTPUT_BASE, char_name, "マシュマロ")

    # マシュマロフォルダのみ削除・再作成
    if os.path.exists(marsh_dir):
        print(f"[DELETE] {marsh_dir}")
        shutil.rmtree(marsh_dir)
    os.makedirs(marsh_dir, exist_ok=True)

    print(f"[INPUT] {marshmallow_path}")

    img = cv2.imread(marshmallow_path, cv2.IMREAD_UNCHANGED)
    if img is None:
        raise FileNotFoundError(f"Cannot read: {marshmallow_path}")

    stickers = detect_and_extract_stickers(img)
    print(f"[DETECT] {len(stickers)} stickers")

    for i, sticker in enumerate(stickers, start_num):
    ...
    print(f"\n[DONE] {len(stickers)} marshmallow stickers saved to {marsh_dir}")
    return len(stickers)
...
if __name__ == '__main__':
    ...
    # 1. ぷるるんマシュマロのみ再処理（ボンドロは維持）
    print("\n" + "="*80)
    print("STEP 1: Pururun Marshmallow Only")
    print("="*80)
    pururun_marsh = os.path.join(resized_dir, "pururun-marshmallow-transparent.png")
    process_marshmallow_only("ぷるるん", pururun_marsh, start_num=16)

    # 2. にじたま両方処理
    print("\n" + "="*80)
    print("STEP 2: Nijitama Full Process")
    print("="*80)
    nijitama_bondro = os.path.join(resized_dir, "nijitama-bondro-transparent.png")
    nijitama_marsh = os.path.join(resized_dir, "nijitama-marshmallow-transparent.png")
    process_both("にじたま", nijitama_bondro, nijitama_marsh)
